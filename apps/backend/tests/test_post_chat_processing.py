"""Tests for post_chat_processing — preview and title updates."""
...
# --- _update_title via update_session_metadata ---


@pytest.mark.asyncio
async def test_title_updated_from_vehicle_tool_call():
    """When set_vehicle tool was called, title is set from vehicle info."""
    session = MagicMock()
    session.auto_title = True
    session.title = DEFAULT_BUYER_TITLE

    deal_state = MagicMock()
    deal_state.vehicle_year = 2024
    deal_state.vehicle_make = "Honda"
    deal_state.vehicle_model = "Civic"
    deal_state.vehicle_trim = "EX"
...
@pytest.mark.asyncio
async def test_title_not_updated_when_auto_title_false():
...
async def test_title_llm_fallback_when_still_default(mock_generate):
...
async def test_title_not_regenerated_when_already_non_default(mock_generate):
...
async def test_dealer_sim_default_title_triggers_llm(mock_generate):
