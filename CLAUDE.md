...
- Message construction: `build_context_message()` produces a per-turn context block (deal state, linked sessions, temporal grounding with the current UTC date). `build_messages()` merges this context into the user message as content blocks (no synthetic assistant reply), keeping the message history clean for caching.
