...
- **Components** (`components/`) — Chat (bubbles, input, voice, ContextPicker, CopyableBlock), Chats (SessionCard with phase dot, preview, deal summary), Insights Panel (`insights-panel/`: AI-driven card-based layout with AiCard base renderer, BriefingCard, NumbersCard, AiVehicleCard, WarningCard, TipCard, SuccessCard, AiChecklistCard, AiComparisonCard, CompactPhaseIndicator, PanelMarkdown, QuickActions), Shared (AppCard with compact prop, buttons, pills, menu)
...
- Inline editing on NumbersCard, AiVehicleCard, and dealer name — corrections sent as structured payloads (vehicle_corrections/deal_corrections) to `PATCH /api/deal/{session_id}` with automatic Haiku re-assessment
