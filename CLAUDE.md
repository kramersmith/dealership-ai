...
- Assessment safety net: if Claude updates numbers but doesn't call `update_deal_health` or `update_red_flags`, the backend runs a Haiku assessment (`assess_deal_state`) to fill in health status and red flags
...
- Backend enums (`app/models/enums.py`): UserRole, SessionType, MessageRole, DealPhase, ScoreStatus, BuyerContext, HealthStatus, RedFlagSeverity, GapPriority, Difficulty (all `StrEnum`)
...
- **Components** (`components/`) — Chat (bubbles, input, voice, ContextPicker, CopyableBlock), Chats (SessionCard with phase dot, preview, deal summary), Insights (DealHealthCard, RedFlagsCard, KeyNumbers, InformationGapsCard, SavingsSummary, phase, scorecard, vehicle, checklist, timer, quick actions), Shared (cards, buttons, pills, menu)
...
- **API** (`lib/`) — API client connecting to the FastAPI backend (no mock layer), `snakeToCamel` utility for mapping backend snake_case fields to frontend camelCase, `dealComputations.ts` for derived deal metrics (savings calculations)
...
- InsightsPanel uses data-driven composition via `getPanelWidgets()` — widgets appear/hide based on available deal data rather than static context-based ordering
- Inline editing on VehicleCard and KeyNumbers — users can tap to correct AI-extracted values, with debounced sync to backend via `PATCH /api/deal/{session_id}` and automatic Haiku re-assessment
- DealStore: `correctNumber` and `correctVehicleField` actions with debounced backend sync
