...
- **Components** (`components/`) — Chat (bubbles, input, voice, ContextPicker, CopyableBlock), Chats (SessionCard with phase dot, preview, deal summary), Insights (HeroSection with deal health + offer delta + AI recommendation + compact phase indicator, RedFlagsCard, KeyNumbers, InformationGapsCard/"Blind Spots", SavingsSummary, scorecard with tap-to-expand, vehicle, checklist, timer with contextual tips, quick actions), Shared (AppCard with compact prop, buttons, pills, menu)
...
- InsightsPanel uses tiered layout via `getPanelLayout()` — hero tier (always visible), alert tier (red flags/blind spots), primary tier (savings/numbers), secondary tier (vehicle/scorecard/checklist), with staggered entrance animations
