...
- **Components** (`components/`) — Chat (bubbles with FailedMessageFooter retry, input with stop generation button and `surfaceVariant` (docked/floating), voice, ChatComposerOverlay (desktop: absolute-positioned floating composer with tray height measurement; mobile: inline flow), ContextPicker (situation cards + VIN submit flow), context pressure banner, `system`-role compaction notices in ChatBubble, CopyableBlock, QuotedCardPreview, QueuePreviewCard (shows queued messages above the input), VinAssistCard, MultiVinAssistCard (multi-VIN intercept flow), VinInterceptModal, `markdownRenderer.tsx` (chat markdown pipeline with GFM table support — parses markdown tables out of the stream and delegates to `VehicleComparisonTable` for responsive horizontal scrolling with sticky label column; falls back to `react-native-markdown-display` for non-table content), `vinAssistUtils.ts` (VIN detection + normalization helpers)), Chats (SessionCard with phase dot, preview, deal summary), Insights Panel (`insights-panel/`: AI-driven card-based layout with AiCard base renderer (entire body taps to reply, with desktop hover-lift), CardReplyInput, CardTitle (shared uppercase muted label), SituationBar (negotiation context display), ThinkingIndicator (pulsing indicator during AI processing), DesktopInsightsDockControl (expand pill + collapsed-updating preview with live/paused label; collapse button moved into InsightsPanel header via `headerAccessory` prop), InsightPanelPreviewIcons (deduped icon strip for collapsed previews) + InsightsPreviewItemChip (per-row chip rendering), panel interruption notice with refresh button, BriefingCard, NumbersCard, AiVehicleCard (expandable container with Specs/Title Check/Market Value sections, VIN prompt), VehicleIntelligencePanel, WarningCard, TipCard, SuccessCard, AiChecklistCard (read-only with progress bar), AiComparisonCard, PanelMarkdown), Shared (AppCard with compact prop, ScreenHeader (animated header with ScrambleText title + icon slots), HeaderIconButton, HoverLiftFrame (desktop hover lift effect), ScrambleText (character-by-character text reveal animation), `VehicleComparisonTable` (shared responsive comparison table used by chat markdown tables and the AiComparisonCard), buttons, pills, menu)
...
- **Hooks** (`hooks/`) — useChat (orchestrates messages + tool calls with event-based SSE parsing including `turn_started`, `interrupted`, `panel_interrupted`, `compaction_*`, optimistic rollback; exposes `stopGeneration`), useEditableField (inline editing with debounced backend sync), useScreenWidth (responsive breakpoint), useIconEntrance (animated icon transitions between screens), useDesktopChatTransition (desktop-specific animated chat/insights panel transitions with fade/slide), useDesktopInsightsShell (desktop panel shell state machine: expanded/collapsed_updating/collapsed_idle/hidden with launcher animations), useDesktopPanelPreference (client-local `localStorage` persistence for desktop panel collapsed state — not server-synced), useSlideIn (reusable slide-in animation), `useInsightsAnimations` (shared analyzing-pulse loop, signature-entrance fade+slide, and finish-flash sequence helpers — consumed by both the mobile collapsed strip and the desktop dock launcher to prevent animation drift), useWebAriaHiddenFocusWorkaround (web a11y shim that blurs focus inside `aria-hidden` subtrees when RN Web Modal opens; pure DOM helper extracted to `lib/webAriaHiddenFocus.ts` for unit tests)
- **API** (`lib/`) — API client connecting to the FastAPI backend (no mock layer). `apiClient.ts` uses a shared `streamBuyerChatSse()` parser for both normal sends and branch sends, preserves structured 4xx detail when safe, hides raw 5xx/proxy bodies behind generic messages, and treats SSE `error` after `done` as a non-fatal warning. Also: `snakeToCamel` utility for mapping backend snake_case fields to frontend camelCase, `dealComputations.ts` for derived deal metrics (savings, `computeOfferDelta`, `getNextActionRecommendation`), vehicle intelligence API functions (upsert-from-vin, decode-vin, confirm-identity, check-history, get-valuation), `headerTitles.ts` (vehicle-aware header title resolution with decode/session/fallback priority), `chatLayout.ts` (chat screen layout constants and helpers — desktop/mobile composer tray, insights sheet insets/radii, chat rail max width, queue preview positioning), `insightsCollapsedPreview.ts` (pure preview-row priority logic and panel-icon dedupe shared by mobile strip + desktop dock — covered by `insightsCollapsedPreview.test.ts`), `insightsPanelCardOrder.ts` (filters comparison/trade_off and orders phase-first), `insightsPanelTypography.ts` (shared text style props for AiCard variants and PanelMarkdown), `dev/mockPanelUpdates.ts` (dev-only mock tool calls for testing panel animations; also exports `DEV_COLLAPSE_DESKTOP_INSIGHTS_EVENT` used by `reviewDesktopDockAnimations()` to drive the dock launcher in dev), `webModalFocus.ts` / `webAriaHiddenFocus.ts` (web modal focus helpers), `modalWebTypography.ts` (web-only font-family style helper so RN Web Modal portals inherit the app font)
...
- InsightsPanel renders AI-generated cards (`ai_panel_cards` on deal state) from the backend-owned canonical panel contract (`kind`, `template`, `title`, `content`, `priority`) — replacing the previous fixed-widget tiered layout. Panel controls (Live/Paused toggle, refresh, and an optional `headerAccessory` for close/collapse buttons) live in a top header bar; SituationBar displays the negotiation context (stance + situation) above cards when present.
...
- Card reply system: tapping the body of any insight card opens a CardReplyInput slide-in drawer (full-card press target with desktop hover-lift; replaces the prior MessageCircle icon affordance); submitting sends a chat message with quoted card context (`QuotedCard` type on Message, rendered as `QuotedCardPreview` in chat bubbles). `chatStore.sendMessage` accepts an optional `quotedCard` param and a `source` param (`QueueMessageSource`: `'typed'` | `'card_reply'` | `'retry'`).
...
- `APP_NAME` constant ('DealershipAI') and other constants (`WEB_FONT_FAMILY`, `TIMER_TIPS`, `SCORE_DESCRIPTIONS`, `MAX_INSIGHTS_PREVIEW_ITEMS`, `CHAT_BUBBLE_MAX_WIDTH`, `DESKTOP_ASSISTANT_BUBBLE_MAX_WIDTH`, `CHAT_VIEW_MAX_WIDTH`) in `lib/constants.ts`
