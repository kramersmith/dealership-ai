...
│   │   │   │                    # (negotiation context), DesktopInsightsDockControl
│   │   │   │                    # (collapse/expand pill + updating preview),
│   │   │   │                    # BriefingCard, NumbersCard,
...
│       │       ├── buyer_chat_stream.py  # Shared buyer-turn SSE pipeline for /message and /branch (chat phase only)
│       │       ├── insights_followup.py  # Detached buyer insights follow-up SSE pipeline + durable job lifecycle
...
│       │       ├── panel_update_service.py  # Panel update policy resolution (per-user insights mode) + on-demand refresh via shared follow-up pipeline
│       │       ├── user_settings.py         # get_or_create_user_settings, settings response builder
...
**user_settings** — (id, user_id [unique FK → users], insights_update_mode [InsightsUpdateMode enum: live/paused], created_at, updated_at). One-to-one with users; created lazily on first access via `get_or_create_user_settings()`. Desktop panel collapse state is NOT stored here — it lives client-side in `localStorage` (ADR 0024).
...
**insights_followup_jobs** — durable follow-up job rows keyed by `(session_id, assistant_message_id, kind)` for detached insights work. Tracks overall follow-up `status`, per-step status (`reconcile_status`, `panel_status`), attempts, cancellation/error details, merged usage, and timestamps.
...
- Numbers: msrp, invoice_price, listing_price, your_target, walk_away_price, current_offer, monthly_payment, apr, loan_term_months, down_payment, trade_in_value (note: `your_target` and `walk_away_price` are written only via the narrow-scope `set_buyer_targets` tool — they represent explicit buyer-stated commitments, never the model's own recommendations; ADR 0026)
- Custom numbers: custom_numbers JSON array of free-form number rows (fees, add-ons, tax, rebates, retail references) shown on the Numbers panel card. Persisted as a full-list replacement via the `update_deal_custom_numbers` tool (migration `0008_deal_custom_numbers.py`).
...
| `AiCardKind` | `vehicle`, `numbers`, `phase`, `warning`, `notes`, `comparison`, `checklist`, `success`, `what_changed`, `dealer_read`, `your_leverage`, `next_best_move`, `if_you_say_yes`, `trade_off`, `savings_so_far` (note: `comparison` and `trade_off` are no longer emitted to the panel — they render as markdown tables in chat per ADR 0018; gaps merge into `checklist.open_questions` at render time) |
| `AiCardPriority` | `critical`, `high`, `normal`, `low` |
| `NumberHighlight` | `good`, `bad`, `neutral` (row-level highlight on `numbers` / `custom_numbers` panel cards) |
| `InsightsUpdateMode` | `live`, `paused` |
| `InsightsFollowupKind` | `linked_reconcile_panel` |
| `InsightsFollowupStatus` | `pending`, `running`, `succeeded`, `failed`, `cancelled` |
| `InsightsFollowupStepStatus` | `pending`, `running`, `succeeded`, `failed`, `skipped`, `cancelled` |
...
```
POST   /auth/signup                     # Sign up (returns token + user settings)
POST   /auth/login                      # Log in (returns token + user settings)
GET    /auth/settings                   # Read persisted user settings (insights update policy)
PATCH  /auth/settings                   # Update persisted user settings

POST   /chat/{session_id}/user-message # Pre-persist user message (no stream) — used by VIN intercept gated flows (ADR 0019)
POST   /chat/{session_id}/message    # Send message → SSE chat stream (turn_started/text/tool_result/retry/step/error + terminal done|interrupted + compaction_*); accepts optional existing_user_message_id to resume on the latest pre-persisted row
POST   /chat/{session_id}/insights-followup # Detached insights follow-up SSE stream for a persisted assistant turn
POST   /chat/{session_id}/messages/{message_id}/branch # Edit-from-here: delete messages after anchor when any; always reset commerce/usage/compaction; then same chat SSE stream (ADR 0020)
...
**Buyer turn lifecycle (ordered SSE, `done` vs panel, client tool flush):** see [buyer-chat-turn.md](buyer-chat-turn.md).
...
**Shared buyer-turn service:** `app/services/buyer_chat_stream.py` owns the shared chat-phase SSE pipeline for both `POST /message` and `POST /messages/{id}/branch`: compaction, user-row insert/update, chat step loop, assistant persistence, chat-phase usage aggregation, and final metadata updates. The branch route performs its prepare/reset transaction first (`session_branch.py`) and then hands control to the same turn streamer. The route layer resolves the user's insights policy up front so paused turns can withhold persistence-affecting buyer tools.

**Detached insights follow-up:** `app/services/insights_followup.py` owns the follow-up SSE pipeline for `POST /chat/{id}/insights-followup`. It creates or reuses a durable `insights_followup_jobs` row for a persisted assistant message, then runs the **panel-only** phase (the reconcile LLM pass was removed per ADR 0026 — `reconcile_status` is always marked `SKIPPED`; the column and `InsightsFollowupKind.LINKED_RECONCILE_PANEL` name are kept for migration/job-key stability). Panel generation is split (ADR 0026): `panel_card_builder.py:build_rendered_panel_cards()` deterministically renders ~9 card kinds (`phase`, `numbers`, `warning`, `checklist`, `your_leverage`, `vehicle`, `success`, `savings_so_far`, `notes`) from deal state with no LLM (the `checklist` card may include `open_questions` from gaps plus playbook `items`), while `panel.py` runs a narrow Sonnet synthesis (prompt `GENERATE_AI_PANEL_SYNTHESIS_PROMPT`) that only produces the 3 genuinely narrative kinds (`dealer_read`, `next_best_move`, `if_you_say_yes`). Rendered + synthesized cards merge and flow through `canonicalize_panel_cards` + `_enforce_single_vehicle_focus_for_panel_cards`. On synthesis retry exhaustion the rendered cards are still delivered. Follow-up usage is merged back into the assistant row and session ledger. The explicit `POST /chat/{id}/panel-refresh` path reuses this same pipeline against the latest assistant turn, but forces a rerun instead of short-circuiting on a previously succeeded job.

**Extraction architecture:** The backend uses a two-pass extraction approach with parallel subagents:
1. **Factual extractor** — extracts structured data (vehicle, deal numbers, scorecard, phase, buyer context, checklist) from conversation
...
7. Step-control logic (`chat_tool_choice_for_step` in `claude/tool_policy.py`, simplified per ADR 0026) bounds tool rounds per buyer message: step 0 uses `auto` tool choice, step ≥ 1 forces `none` (text-only) to guarantee a user-visible reply on the rare continuations. Additionally, `chat_loop_engine.run_chat_loop_engine` short-circuits the turn when step 0 produces a substantive pre-tool reply (≥150 chars) alongside tool calls, skipping the continuation step entirely (TIMING outcome `reply_with_tools`). This reflects the complete-reply-first prompt design — the model writes its full reply BEFORE emitting tool_use blocks since tool results are fire-and-forget and don't round-trip back to the model.
8. The shared buyer-turn service persists the assistant message before emitting `done`, so the delivered reply already has a durable row before the chat stream closes. If that assistant persistence fails, the service emits a safe `error` event and removes any newly inserted orphan user row.
9. When the step loop reaches a text-only completion, the backend emits `done` with `assistant_message_id` so input can unblock and the client can start detached follow-up if needed.
10. After `done`, the frontend opens `POST /chat/{id}/insights-followup` for completed buyer replies when `insights_update_mode=live`. That detached stream emits `panel_started`, then terminal `panel_done` (atomic canonical `cards` + `assistant_message_id`) or `panel_error`. If `insights_update_mode=paused`, the client skips the automatic follow-up and users trigger `POST /chat/{id}/panel-refresh` explicitly for panel reruns (the reconcile LLM pass was removed per ADR 0026).
11. Panel `max_tokens` starts at 4096 and escalates to 8192 on bounded truncation retries. The canonical panel contract uses per-kind instance caps (`vehicle` up to 6, all other kinds capped at 1) with identity-based dedupe (VIN for vehicles, `kind` for everything else) — there is no global panel length cap. Panel kinds `comparison` and `trade_off` are filtered out of panel output: side-by-side comparisons render as markdown tables in chat. A `phase` card (negotiation stance + situation) is always first. Panel single-focus enforcement collapses vehicle cards to the active vehicle when there is one shopping vehicle or an explicit focus signal (ADR 0018).
12. Backend folds chat-phase usage into the assistant row on send, then merges detached follow-up usage into the same assistant row and session-level usage ledger when follow-up completes.
...
16. Frontend `apiClient.ts` uses a shared `streamBuyerChatSse()` parser for both normal sends and branch sends, and a separate `streamInsightsFollowupSse()` parser for detached post-response work. The send parser preserves structured backend 4xx details, hides raw 5xx/proxy bodies behind generic messages, treats `error` events before `done` as fatal, and treats `error` events after `done` as non-fatal warnings so the delivered reply stays on screen. Main-turn tool result callbacks are still deferred until after `done`; the detached follow-up stream only emits the lifecycle events `panel_started` / `panel_done` / `panel_error` (no reconcile tool_results since ADR 0026).
...
18. **Client-side message queue:** Users can send follow-up messages while the AI is processing. Messages are queued in the Zustand chat store (`queueBySession`) and dispatched FIFO after each turn completes. QueuePreviewCard renders up to 3 pending items above the composer. ChatInput remains enabled. Branch edit is blocked while the queue has pending items. See ADR 0022.
...
5. Dashboard panel ordering and system prompt preamble adapt to the selected context
...
- **Claude Sonnet 4.6** (`claude-sonnet-4-6`) — primary model for chat, with max_tokens: 4096 and history truncated to last 20 messages. **Claude Haiku 4.5** (`claude-haiku-4-5-20251001`) — fast model for lightweight tasks (session title generation and deal assessment safety net)
...
| 3 | Remaining dashboard UI — scorecard, vehicle card, checklist, timer, session linking | 8-10 |
