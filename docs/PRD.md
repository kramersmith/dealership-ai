...
**Supporting components:** AiCard (base card renderer; entire card body acts as the reply affordance with desktop hover-lift), CardReplyInput (slide-in reply drawer), CardTitle (shared uppercase muted label component), SituationBar (negotiation context display with stance badge and situation summary), PanelMarkdown (markdown in cards), InsightPanelPreviewIcons + InsightsPreviewItemChip (deduped icon strip and per-row chips for the collapsed mobile strip / desktop dock preview).

**Card reply system:** Tapping any insight card body opens a slide-in input drawer attached to the card. Submitting sends a chat message with the card context quoted, so the AI can respond with full awareness of what the user is referencing. A QuotedCardPreview renders a compact summary of the referenced card inside the user's chat bubble.
